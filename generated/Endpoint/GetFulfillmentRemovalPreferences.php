...
class GetFulfillmentRemovalPreferences extends \Em411\Allegro\Api\Runtime\Client\BaseEndpoint implements \Em411\Allegro\Api\Runtime\Client\Endpoint
{
     ...
     *
     * @throws \Em411\Allegro\Api\Exception\GetFulfillmentRemovalPreferencesUnauthorizedException
     * @throws \Em411\Allegro\Api\Exception\GetFulfillmentRemovalPreferencesForbiddenException
     * @throws \Em411\Allegro\Api\Exception\GetFulfillmentRemovalPreferencesNotFoundException
     */
    protected function transformResponseBody(\Psr\Http\Message\ResponseInterface $response, \Symfony\Component\Serializer\SerializerInterface $serializer, ?string $contentType = null)
    {
        ...
        if ((null === $contentType) === false && (401 === $status && false !== mb_strpos(strtolower($contentType), 'application/vnd.allegro.public.v1+json'))) {
            throw new \Em411\Allegro\Api\Exception\GetFulfillmentRemovalPreferencesUnauthorizedException($serializer->deserialize($body, 'Em411\Allegro\Api\Model\AuthError', 'json'), $response);
        }
        if (403 === $status) {
            throw new \Em411\Allegro\Api\Exception\GetFulfillmentRemovalPreferencesForbiddenException($response);
        }
        if (404 === $status) {
            throw new \Em411\Allegro\Api\Exception\GetFulfillmentRemovalPreferencesNotFoundException($response);
        }
    }
}
