...
class DictionaryCategoryProductParameterNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    use CheckArray;
    ...
    {
        return \Em411\Allegro\Api\Model\DictionaryCategoryProductParameter::class === $type;
    }

    public function supportsNormalization(mixed $data, ?string $format = null, array $context = []): bool
    {
        return \is_object($data) && \Em411\Allegro\Api\Model\DictionaryCategoryProductParameter::class === \get_class($data);
    }
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        $object = new \Em411\Allegro\Api\Model\DictionaryCategoryProductParameter();
        ...
        if (\array_key_exists('restrictions', $data) && null !== $data['restrictions']) {
            $object->setRestrictions($this->denormalizer->denormalize($data['restrictions'], \Em411\Allegro\Api\Model\DictionaryCategoryProductParameterrestrictions::class, 'json', $context));
            unset($data['restrictions']);
        } elseif (\array_key_exists('restrictions', $data) && null === $data['restrictions']) {
            $object->setRestrictions(null);
        }
        if (\array_key_exists('dictionary', $data) && null !== $data['dictionary']) {
            $values = [];
            foreach ($data['dictionary'] as $value) {
                $values[] = $this->denormalizer->denormalize($value, \Em411\Allegro\Api\Model\DictionaryCategoryProductParameterdictionaryItem::class, 'json', $context);
            }
            $object->setDictionary($values);
            unset($data['dictionary']);
        } elseif (\array_key_exists('dictionary', $data) && null === $data['dictionary']) {
            $object->setDictionary(null);
        }
    ...
    {
        return [\Em411\Allegro\Api\Model\DictionaryCategoryProductParameter::class => false];
    }
}
