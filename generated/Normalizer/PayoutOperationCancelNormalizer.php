...
class PayoutOperationCancelNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    use CheckArray;
    ...
    {
        return \Em411\Allegro\Api\Model\PayoutOperationCancel::class === $type;
    }

    public function supportsNormalization(mixed $data, ?string $format = null, array $context = []): bool
    {
        return \is_object($data) && \Em411\Allegro\Api\Model\PayoutOperationCancel::class === \get_class($data);
    }
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        $object = new \Em411\Allegro\Api\Model\PayoutOperationCancel();
        ...
        if (\array_key_exists('payout', $data) && null !== $data['payout']) {
            $object->setPayout($this->denormalizer->denormalize($data['payout'], \Em411\Allegro\Api\Model\Payout::class, 'json', $context));
            unset($data['payout']);
        } elseif (\array_key_exists('payout', $data) && null === $data['payout']) {
            $object->setPayout(null);
        }
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if ($data->isInitialized('payout') && null !== $data->getPayout()) {
            $dataArray['payout'] = $this->normalizer->normalize($data->getPayout(), 'json', $context);
        }
        foreach ($data as $key => $value) {
            if (preg_match('/.*/', (string) $key)) {
                $dataArray[$key] = $value;
    ...
    {
        return [\Em411\Allegro\Api\Model\PayoutOperationCancel::class => false];
    }
}
