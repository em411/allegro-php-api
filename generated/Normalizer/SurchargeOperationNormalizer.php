...
class SurchargeOperationNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    use CheckArray;
    ...
    {
        return \Em411\Allegro\Api\Model\SurchargeOperation::class === $type;
    }

    public function supportsNormalization(mixed $data, ?string $format = null, array $context = []): bool
    {
        return \is_object($data) && \Em411\Allegro\Api\Model\SurchargeOperation::class === \get_class($data);
    }
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        $object = new \Em411\Allegro\Api\Model\SurchargeOperation();
        ...
        if (\array_key_exists('surcharge', $data) && null !== $data['surcharge']) {
            $object->setSurcharge($this->denormalizer->denormalize($data['surcharge'], \Em411\Allegro\Api\Model\Surcharge::class, 'json', $context));
            unset($data['surcharge']);
        } elseif (\array_key_exists('surcharge', $data) && null === $data['surcharge']) {
            $object->setSurcharge(null);
        }
        if (\array_key_exists('participant', $data) && null !== $data['participant']) {
            $object->setParticipant($this->denormalizer->denormalize($data['participant'], \Em411\Allegro\Api\Model\BuyerParticipant::class, 'json', $context));
            unset($data['participant']);
        } elseif (\array_key_exists('participant', $data) && null === $data['participant']) {
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if ($data->isInitialized('surcharge') && null !== $data->getSurcharge()) {
            $dataArray['surcharge'] = $this->normalizer->normalize($data->getSurcharge(), 'json', $context);
        }
        if ($data->isInitialized('participant') && null !== $data->getParticipant()) {
            $dataArray['participant'] = $this->normalizer->normalize($data->getParticipant(), 'json', $context);
        }
        foreach ($data as $key => $value) {
            if (preg_match('/.*/', (string) $key)) {
                $dataArray[$key] = $value;
    ...
    {
        return [\Em411\Allegro\Api\Model\SurchargeOperation::class => false];
    }
}
